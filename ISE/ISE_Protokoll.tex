...
\begin{tabular}{|c|c|c|}
\hline \textbf{Bezeichnung} &\textbf{ Betrag} & \textbf{Fehler} \\
\hline Volumen V (gemittelt) & $1145\cdot 10^{-6}$ m & $3\cdot 10^{-6} $ \\  
\hline Kolbenmasse m (gemittelt) & $4,53333\cdot{10^{-3}kg}$ & $0,06\cdot{10^{-3}}$ \\ 
\hline Kolbenquerschnitsfläche S(berechnet) & $ 111,22\cdot 10^{{-6}}m^2 $ & $0,56\cdot 10^{-6}$ \\ 
\hline Raumdruck p (abgelesen) & 100800Pa  & 300Pa \\ 
\hline 
\end{tabular} 
